  ...
  return (
    <section className="bg-paper">
      <Container className="py-20 sm:py-24">
        ...
        />
        <ul className="mt-12 grid gap-px overflow-hidden rounded-sm border border-ink/15 bg-ink/15 sm:grid-cols-2 lg:grid-cols-4">
          {whoWeServe.map((a, i) => (
            <li key={a.label} className="bg-paper">
              <Reveal delay={(i % 4) * 0.05} y={12} className="flex h-full items-center gap-4 p-6">
                <a.icon className="size-7 shrink-0 stroke-[1.4] text-muted" aria-hidden="true" />
